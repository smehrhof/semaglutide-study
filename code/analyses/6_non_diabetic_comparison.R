...
m3_para_non_diabetic_params <- readRDS(here::here("data/model_fits/non_diabetics/m3_para_non_diabetics_params.RDS"))

m3_para_normal_weight_params <- readRDS(here::here("data/model_fits/low_bmi/m3_para_normal_weight_params.RDS"))
...
data <- bind_rows(
  ...
  non_diabetic_data$demographic_data %>% 
    select(subj_id, age, gender, bmi, antidepressant) %>%
    mutate(antidepressant = ifelse(is.na(antidepressant), 0, antidepressant)) %>% 
...
data %<>% 
  ungroup %>% 
  mutate_at(colnames(data)[c(5, 12:15, 18:19)], rescale)

# Visualize distributions
ggplot(gather(data %>% select(colnames(data)[c(5, 12:15, 18:19)])) %>% na.omit(), 
       aes(value)) + 
...
data %>% 
  ...
  cohens_d(estimate_a ~ group, var.equal = TRUE)
...
a_plot <- raincloud_plot(dat = data, title = "", 
 ...
 # theme(axis.text.y = element_text(angle = 25)) +
  ...
  annotate(x=0.5, xend=0.5, y=0, yend=1, colour="black", lwd=0.75, geom="segment")



### FINDRISC predicting choice bias across groups
