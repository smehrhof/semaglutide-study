...
transformed parameters {
  ...
  matrix[N_time, N] a_tilde;

  // Individual-level parameters
  matrix<lower=0, upper=4>  [N, N_time] kE;
  matrix<lower=0, upper=4>  [N, N_time] kR;
  matrix<lower=-10, upper=10> [N, N_time] a;
  ...
  a_tilde   = diag_pre_multiply(a_sd, L_R_a)      * a_pr;

  // Compute individual-level parameters from non-centered parameterization
  for (time in 1:N_time){
    for (i in 1:N) {
      kE[i, time]   = Phi_approx(kE_mean[time] + kE_tilde[time,i]) * 4;
      kR[i, time]   = Phi_approx(kR_mean[time] + kR_tilde[time,i]) * 4;
      a [i, time]   = (Phi_approx(a_mean[time] + a_tilde[time,i])  * 20) - 10;
    }
  }
}
  ...
  for(time in 1:N_time){
    for(i in 1:N){
        ...
        for(t in 1:(T_subj[i, time])){
            sv_a   = (kR[i, time] * amount_a[i, time, t])  - kE[i, time] * (effort_a[i, time, t]);
            sv_b   = (kR[i, time] * amount_b[i, time, t])  - kE[i, time] * (effort_b[i, time, t]);
            choice[i, time, t] ~ bernoulli_logit(1 * (a[i, time] + (sv_a - sv_b)));
        }
    }
  }
}
  ...
  for (time in 1:N_time){
    for (i in 1:N) {
      ...
      for (t in 1:(T_subj[i, time])) {
        ...
        log_lik[i, time, t] += bernoulli_logit_lpmf(choice[i, time, t] | 1 * (a[i, time] + (sv_a - sv_b)));

        post_pred[i, time, t] = bernoulli_rng(inv_logit(1 * (a[i, time] + (sv_a - sv_b))));
      }
    }
  }
  }
