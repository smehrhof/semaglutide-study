...
transformed parameters {
  ...
  vector<lower=-2, upper=0>[N] a;

  for (i in 1:N) {
    kE[i]    = (Phi_approx(mu_pr[1] + sigma[1] * kE_pr[i]) * 1);
    a[i]    = (Phi_approx(mu_pr[2] + sigma[2] * a_pr[i]) * 2) - 2;
  }
}
...
// Priors
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {
      sv_a   = amount_a[i, t] * exp(-kE[i] * effort_a[i, t]);
      sv_b   = amount_b[i, t] * exp(-1 * effort_b[i, t]);
      choice[i, t] ~ bernoulli_logit(5 * ((a[i]) + (sv_a - sv_b)));
    }
  }
}
generated quantities {
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:(Tsubj[i])) {
        sv_a   = amount_a[i, t] * exp(-kE[i] * effort_a[i, t]);
        sv_b   = amount_b[i, t] * exp(-kE[i] * effort_b[i, t]);
        log_lik[i] += bernoulli_logit_lpmf(choice[i, t] | 5 * ((a[i]) + (sv_a - sv_b)));

        // generate posterior prediction for current trial
        y_pred[i, t] = bernoulli_rng(inv_logit( 5 * ((a[i]) + (sv_a - sv_b))));
      }
    }
  }
}
