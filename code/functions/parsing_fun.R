...
parsing <- function(file,
  ...
  suppressMessages({
    ...
    for(subj in comp_1_id){
      ...
      if(session == "control"){
        raw_dat[[subj]] %<>% 
          ...
          naniar::replace_with_na_at(.vars = c("diabetes_med"),
                             condition =  ~ ! .x %in% 0:1) %>% 
        naniar::replace_with_na(., replace = list(diabetes_med_type = "1", diabetes_med_type = "0"))  
      }
      ...
      comp_1_dat_subj <- raw_dat[[subj]] %>%
        { if(session == "s1"){
          ...
          select(., c(prolific_id, glp_treatment, diabetes_med, other_med, games, time_response)) 
        }} %>% 
          ...
          add_column(
            glp_dose_mg = { ifelse(.$type_glp == "Semaglutide_Ozempic",
                                   { ifelse(.$prolific_id %in% raw_doses_dat$prolific_id, 
                                            ...
                                            na.omit(raw_dat[[subj]]["glp_dose"])[1,1])},
                                   NA) }, 
            last_injection_days = { ifelse(.$glp_treatment == "Current", 
                                           na.omit(raw_dat[[subj]]["last_injection"])[1,1], 
                                           NA) }, 
            last_injection_time = { ifelse(.$glp_treatment == "Current", 
                                           case_when(na.omit(raw_dat[[subj]]["last_injection_time"]) == 0 ~ "before 6am", 
                                                     na.omit(raw_dat[[subj]]["last_injection_time"]) == 1 ~ "6am - 10am", 
                                                     na.omit(raw_dat[[subj]]["last_injection_time"]) == 2 ~ "10am - 2pm", 
                                                     na.omit(raw_dat[[subj]]["last_injection_time"]) == 3 ~ "2pm - 6pm", 
      ...
      if(session == "s1"){
        comp_1_dat_subj %<>% 
          ...
          add_column(
            ...
            .after = "other_med") %>% 
          add_column(
            contraceptive_med = { ifelse(.$other_med == 1, 
                                         { ifelse(str_detect(.$other_med_type, "Contraceptive"), 
                                                1, 0) }, 
                                         NA)
              }, 
            antidepressant_med = { ifelse(.$other_med == 1, 
                                         { ifelse(str_detect(.$other_med_type, "Antidepressant"), 
                                                  1, 0) }, 
                                         NA)
            }, 
            antidepressant_med_type = { ifelse(.$other_med == 1, 
                                          { ifelse(str_detect(.$other_med_type, "Antidepressant"), 
                                                   str_split(na.omit(raw_dat[[subj]]["other_med_text"])[1,1], ", ")[[1]][str_detect(str_split(.$other_med_type, ", ")[[1]], "Antidepressant")],
                                                   NA) }, 
                                          NA)
            }, 
        ...
        comp_1_dat_subj %<>% 
          # diabetes medication
          mutate(
            diabetes_med = ifelse(.$diabetes_med == 1, 
                                  ifelse(any(str_detect(na.omit(raw_dat[[subj]]["diabetes_med_type"])$diabetes_med, 
                                                        "None")), 
                                         0, 1),
                                  0)) %>%
          add_column(
            ...
            diabetes_med_type_other = { ifelse(.$diabetes_med == 1, 
                                               ifelse(na.omit(raw_dat[[subj]]["diabetes_medication_type_other"]), 
                                                      na.omit(raw_dat[[subj]]$diabetes_medication_type_other), 
                                                      NA), 
                                               NA) },
            .after = "diabetes_med") %>%
          # other daily medication
          add_column(
            ...
            .after = "other_med") %>% 
          add_column(
            contraceptive_med = { ifelse(.$other_med == 1, 0, NA)}, 
            antidepressant_med = { ifelse(.$other_med == 1, 0, NA)}, 
            antidepressant_med_type = { ifelse(.$other_med == 1, NA, NA)}, 
            .after = "other_med_type") 
      }
    ...
    return(list(
      ...
      # (2): Task meta data
      "task_meta_dat" = comp_2_dat_meta, 
      # (3): Task data
      "task_dat" = comp_2_dat_task, 
